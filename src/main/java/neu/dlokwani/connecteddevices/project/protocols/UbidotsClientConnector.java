...
//	private String ubidots_topic = "/v1.6/devices/tempmonitor/tempactuator/lv";
			...
			try {
				...
				if (tls_flag == true) {
					try {
						...
						log.log(Level.INFO, "TLS Configured with" + server_url);
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
	 ...
	 */
	public boolean ubidots_mqtt_subscribe(String topic) {
		try {
	...
	public void messageArrived(String topic, MqttMessage message) throws Exception {
		...
		UbidotsClientConnector ubidotsConnector = new UbidotsClientConnector();
		 if ((topic).equals("/v1.6/devices/pedestrian-project/led-actuator/lv")) {
			log.info("message from UBIDOTS for LED ACTUATOR:  " + message.toString());
			System.out.println("ACTUATOR   :  " + message.toString());
			byte[] payload = message.getPayload();
			ubidotsConnector.ubidots_mqtt_connect();
			ubidotsConnector.publishMessage("ActFromGD", 2, payload);
		}
	}
